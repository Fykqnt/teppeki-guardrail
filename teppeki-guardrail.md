...
> **ドキュメント構成:** 本ファイルは guardrail サービス本体の構築手順。Next.js chat-ui との統合方法・既知の問題と対策は `teppeki-guardrail-implementation.md` を参照。
...
# python の実行時に requests へモンキーパッチを当てることで一時的に回避
...
# 実行時に HF API へ更新確認に行って MissingSchema エラーが再発するのを防ぐため、オフラインモードを強制
...
## 16. chat-ui 統合のための API コントラクト

chat-ui（Next.js）から呼び出す際のエンドポイント仕様:
...
| ファイル | 内容 |
|---|---|
| `teppeki-guardrail.md`（本ファイル） | guardrail サービス本体の構築手順（ディレクトリ構成・コード・Dockerfile・Cloud Run） |
| `teppeki-guardrail-implementation.md` | Next.js chat-ui 統合（route.ts・409 ハンドリング・Supabase 保存）、既知の問題 11 件と対策 |

**統合時の読み方:** 本ファイルで guardrail を構築 → implementation で route.ts 変更・フロントエンド対応・既知問題の確認。
