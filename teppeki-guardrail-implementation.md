...
│   ├── redis_client.py      # Redisセッション管理
│   ├── llm_client.py        # LLM呼び出しラッパー
│   └── models.py            # Pydanticスキーマ
├── redactor/
│   ├── __init__.py
│   ├── redactor.py          # PIIマスキング本体（presidioプロジェクトからコピー）
│   └── config.py            # 閾値・パターン・許可リスト（同上）
├── tests/
│   ├── test_chat.py
│   └── test_redactor.py
├── Dockerfile
...
`redactor.py` に `redact_text_with_mapping()` 関数が存在することを確認してください。
この関数は以下のシグネチャを持ちます:
...
# GiNZAモデルをビルド時にダウンロード（コールドスタート対策）
...
presidio の `redactor.py` に同関数がない場合、以下を追加してください（Issue 6 の位置ベース置換対応済み）:
